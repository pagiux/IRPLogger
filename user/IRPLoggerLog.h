...
//  Structure for managing current state.screen_dump
typedef struct _IRPLOG_CONTEXT {
    HANDLE port;
	WCHAR volume_name[MY_MAX_PATH];
	WCHAR dos_name[MY_MAX_PATH];
    BOOLEAN log_to_file;
    FILE   *output_file;
	BOOLEAN (*logs_filter) (PLOG_RECORD);
    // For synchronizing shutting down of both threads
    BOOLEAN cleaning_up;
    HANDLE  shutdown;
} IRPLOG_CONTEXT, *PIRPLOG_CONTEXT;
